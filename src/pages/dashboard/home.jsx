...
function Home() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    axios
      .get("https://health-wellness-backend.onrender.com/goaltrackers/view", {
   ...
   return (
     <div>
       <section className="bg-white p-4 rounded shadow">
         <p className="text-gray-700 text-2xl pt-2 pb-5 font-semibold">
           Welcome user
         </p>
         <div className="flex flex-wrap gap-5 justify-center items-center">
           {activityTypes.map(
             ({ name, icon, bgColor, goalKey, currentKey }) => {
               const goal = activityData?.[goalKey] ?? 0;
               const currentValue = activityData?.[currentKey] ?? 0;
               const percentage = goal
                 ? Math.round((currentValue / goal) * 100)
                 : 0;
               return (
                 <div
                   key={name}
         ...
         <div className="flex gap-4 justify-around my-5 flex-wrap">
           <div className="w-[95%] sm:w-[75%] lg:w-[60%] bg-white shadow-md rounded-lg p-4">
             <div className="flex justify-between items-center">
               <h2 className="text-gray-700 text-xl pt-2 pb-5 font-semibold">
                 Activity
               </h2>
